...
const SOURCE_OPTIONS = [
  ...
  { value: 'line.lineKey', label: 'Line - Key' },
  { value: 'line.sku', label: 'Line — Shopify SKU' },
  { value: 'line.supplierSku', label: 'Line — Supplier SKU' },
  { value: 'line.qty', label: 'Line — Quantity' },
  { value: 'line.previewCdnUrl', label: 'Line - Mockup Preview CDN' },
  { value: 'line.designDriveLink', label: 'Line - Design Drive Link' },
  { value: 'line.crogsPrice', label: 'Line - CROGS Price' },
  { value: 'line.crogsTotal', label: 'Line - CROGS Total' },
      ...
      .then(d => {
        const orders = (d.orders ?? []).slice(0, 3).map((o: any) => ({
          ...
          lines: (o.lines ?? []).map((l: any) => ({
            sku: l.sku,
            lineKey: l.lineKey ?? `${o.shopifyOrderNumber.replace(/^#/, '')}_${l.linePosition || 1}`,
