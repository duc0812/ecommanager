    ...
    <article className="group flex flex-col overflow-hidden rounded-[14px] border border-[#E6E3DE] bg-white transition-all duration-200 hover:-translate-y-0.5 hover:border-[#D6D2CB] hover:shadow-[0_12px_28px_-18px_rgba(27,26,23,0.35)]">
      <div className="relative aspect-[4/5] overflow-hidden bg-[#F2F1EE]">
        {a.mediaUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#B8B2A9]"><span className="material-symbols-outlined text-[36px]">image_not_supported</span></div>
        )}
        {a.mediaType && (
      ...
      <div className="flex flex-1 flex-col gap-[10px] p-[16px]">
        <div className="flex flex-wrap items-center gap-[6px]">
          ...
          <span className="ml-auto font-[family-name:var(--font-plex-mono)] text-[10px] text-[#B8B2A9]">{formatDate(a.startDate)}</span>
        </div>
        ...
        {a.advertiser.pageName && <p className="text-[11.5px] text-[#78716C]">{a.advertiser.pageName}</p>}
        ...
        <div className="mt-auto flex flex-col gap-2 border-t border-[#EFEDE9] pt-[12px]">
          {a.linkUrl && (
            <a href={a.linkUrl} target="_blank" rel="noreferrer" title={a.linkUrl} className="flex items-center gap-[6px] text-[#57534E] transition-colors hover:text-[#3F3AC4]">
              <span className="material-symbols-outlined text-[14px] text-[#B8B2A9]">link</span>
          ...
          {s.adStyle === 'product' && a.productPublishedAt && (
            <p className="font-[family-name:var(--font-plex-mono)] text-[10.5px] text-[#B8B2A9]">Uploaded {formatDate(a.productPublishedAt)}</p>
          )}
          <div className="flex items-center gap-2">
            {a.adLibraryUrl ? (
              <a href={a.adLibraryUrl} target="_blank" rel="noreferrer" title={a.adArchiveId} className="truncate font-[family-name:var(--font-plex-mono)] text-[10.5px] text-[#B8B2A9] hover:text-[#57534E]">#{a.adArchiveId}</a>
            ) : (
