...
function productUrl(p: Product): string | null {
  if (!p.handle) return null
  const d = (p.store?.domain ?? '').replace(/^https?:\/\//, '').replace(/\/+$/, '')
  if (!d) return null
  return `https://${d}/products/${p.handle}`
}
...
export default function ProductCard({ p, onSave, rank, rankDelta }: { p: Product; onSave: (p: Product) => void; rank?: number; rankDelta?: number | null }) {
  ...
  const media = p.imageUrl ? (
    ...
    <img src={p.imageUrl} alt={p.title ?? ''} loading="lazy" className="h-full w-full object-cover transition-transform duration-200 group-hover:scale-[1.03]" />
  ) : (
    ...
    <article className="group relative flex flex-col overflow-hidden rounded-[14px] border border-[#E6E3DE] bg-white transition-all duration-200 hover:-translate-y-0.5 hover:border-[#D6D2CB] hover:shadow-[0_12px_28px_-18px_rgba(27,26,23,0.35)]">
      {rank !== undefined && (
        <div className="absolute left-[10px] top-[10px] z-10 flex items-center gap-[5px]">
          <span className="rounded-[6px] bg-[#1B1A17]/80 px-[8px] py-[4px] font-[family-name:var(--font-plex-mono)] font-medium text-[10px] text-white backdrop-blur-sm">#{rank}</span>
          <TrendBadge delta={rankDelta} />
        </div>
      )}
      {url ? (
        <a href={url} target="_blank" rel="noreferrer" title={p.title ?? 'Mở sản phẩm'} className="block aspect-[4/5] overflow-hidden bg-[#F2F1EE]">{media}</a>
      ) : (
        <div className="aspect-[4/5] overflow-hidden bg-[#F2F1EE]">{media}</div>
      )}
      <div className="flex flex-1 flex-col gap-[8px] p-[16px]">
        {url ? (
          <a href={url} target="_blank" rel="noreferrer" className="line-clamp-2 text-[14.5px] font-semibold leading-[1.3] tracking-[-0.012em] text-[#1B1A17] transition-colors hover:text-[#3F3AC4] hover:underline">{p.title}</a>
        ) : (
        ...
        <div className="mt-auto flex items-center gap-2 border-t border-[#EFEDE9] pt-[12px]">
          {url ? (
            <a href={url} target="_blank" rel="noreferrer" className="flex items-center gap-[5px] text-[#57534E] transition-colors hover:text-[#3F3AC4]">
              <span className="material-symbols-outlined text-[15px] text-[#78716C]">open_in_new</span>
              <span className="text-[11.5px] font-medium">Xem sản phẩm</span>
            </a>
          ) : (
            <span className="text-[11.5px] text-[#78716C]">Không có link</span>
          )}
