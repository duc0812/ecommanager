...
// Written inline under each line by the card builder. The backfill uses it to recognise a
// card that already carries the customer input and leave it alone.
export const PERSONALIZATION_INLINE_LABEL = 'Personalization (customer input):'
...
// True when the card was generated by the current builder, which already interleaves the
// customer input under each line — backfilling it would only append a duplicate copy.
// A card carrying just the backfill marker returns false, so its block can still be refreshed.
export function cardHasInlinePersonalization(desc: string): boolean {
  return desc.includes(PERSONALIZATION_INLINE_LABEL)
}
